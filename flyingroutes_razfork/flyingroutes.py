...
def send_icmp(progress, sender_task, timeout, n_hops, host_ip, queue, sync_queue):
    """
    ...
    for ttl in range(1, n_hops + 1):
        ...
        try:
            ...
            data = pack(
                str(len((FLAG + str(ttl)))) + "s", (FLAG + str(ttl)).encode()
            )  # Data is flag + TTL value (needed for receiver to map response to TTL)
            calc_checksum = icmp_checksum(
                header + data
            )  # Checksum value for header packing
            header = pack("bbHHh", 8, 0, calc_checksum, getpid() & 0xFFFF, 1)
            b_calc_checksum = int.from_bytes(
                calc_checksum.to_bytes(2, "little"), "big"
            )  # Keep checksum value in reverse endianness
            if system() == "Windows":
            ...
            for n in range(packets_to_repeat):  # Send several packets per TTL value
                ...
                send_time = time()
                queue.put(
                    (None, b_calc_checksum, ttl, send_time)
                )  # Store checksum and TTL value in queue for the receiver thread
...
def send_udp(
    ...
    for ttl in range(1, n_hops + 1):
        ...
        try:
            ...
            for n in range(
                packets_to_repeat
            ):  # Send several packets per TTL value but change the destination port (for ECMP routing)
                tx_socket.sendto((FLAG + str(ttl)).encode(), (host_ip, dst_port + n))
                send_time = time()
                queue.put(
                    (None, src_port, ttl, send_time)
                )  # Store source port and TTL value in queue for the receiver thread
                progress.update(sender_task, advance=1)
            tx_socket.close()
        except error as e:
            print(f"Error while setting TTL and sending data: {e}")
...
def send_tcp(
    ...
    for ttl in range(1, n_hops + 1):
        ...
        for n in range(
            ...
            tx_socket = socket(
                AF_INET, SOCK_STREAM
            )  # One socket per destination port (per packet to send)
            bound = False
            while not bound:  # Set source port (try all from 1024 up to 65535)
                try:
                    tx_socket.bind(("", src_port))
            ...
            if system() == "Windows":
                ...
                tx_socket.setsockopt(SOL_IP, IP_TTL, ttl)  # Set TTL value
            tx_socket.setblocking(
                False
            )  # Set socket in non-blocking mode to allow fast sending of all the packets
            src_ports.append(
                src_port
            )  # Store source port used in the list of source ports for the current TTL value
            try:
                ...
                send_time = time()
                sockets.append(
                    (tx_socket, src_ports, ttl, send_time)
                )  # Store socket, source ports and TTL value to check connection status later on
            progress.update(sender_task, advance=1)
    ...
    sleep(
        timeout
    )  # To allow TCP connections to be established (if target is reached by some sockets), still lower than TCP idle timeout
    ...
    for (
        ...
        ttl,
        send_time,
    ) in sockets:  # Test connection status for each socket by trying to send data
        try:
            s.send((FLAG + str(ttl)).encode())  # Try sending TTL value in data
            queue.put(
                (True, src_ports, ttl, send_time)
            )  # Store True to indicate that target was reached with source port and TTL value in queue for the receiver thread
        except Exception as e:
            queue.put(
                (None, src_ports, ttl, send_time)
            )  # Store source port and TTL value in queue for the receiver thread
            s.close()
        s.close()
    ...
    sync_queue.put(
        True
    )  # Indicate to the receiver thread that receiver can continue with mapping of sent responses to sent packets
...
def send_all(
    ...
    for ttl in range(1, n_hops + 1):
        ...
        for n in range(
            ...
            try:
                ...
                send_time = time()
                queue.put(
                    ("icmp", None, b_calc_checksum, ttl, send_time)
                )  # Store checksum and TTL value in queue for the receiver thread
            except error as e:
                print(
                    f"Error while setting TTL and sending ICMP data, continuing with other protocols: {e}"
                )
                tx_socket_icmp.close()
            # UDP
            try:
                tx_socket_udp.sendto(
                    (FLAG + str(ttl)).encode(), (host_ip, dst_port + n)
                )
                send_time = time()
                queue.put(
                    ("udp", None, src_port, ttl, send_time)
                )  # Store source port and TTL value in queue for the receiver thread
            except error as e:
                print(
                    f"Error while setting TTL and sending UDP data, continuing with other protocols: {e}"
                )
                tx_socket_udp.close()
            # TCP
            src_port += 1  # Source port selection per packet to send will allow the receive function to associate sent TCP packets to receive ICMP messages
            tx_socket_tcp = socket(
                AF_INET, SOCK_STREAM
            )  # One socket per destination port (per packet to send)
            bound = False
            while not bound:  # Set source port (try all from 1024 up to 65535)
            ...
            tx_socket_tcp.setblocking(
                False
            )  # Set socket in non-blocking mode to allow fast sending of all the packets
            src_ports_tcp.append(
                src_port
            )  # Store source port used in the list of source ports for the current TTL value
            try:
                ...
                send_time = time()
                tcp_sockets.append(
                    (tx_socket_tcp, src_ports_tcp, ttl, send_time)
                )  # Store socket, source ports and TTL value to check connection status later on
            progress.update(sender_task, advance=1)
    ...
    for (
        ...
        ttl,
        send_time,
    ) in tcp_sockets:  # Test connection status for each socket by trying to send data
        try:
            s.send((FLAG + str(ttl)).encode())  # Try sending TTL value in data
            queue.put(
                ("tcp", True, src_ports_tcp, ttl, send_time)
            )  # Store True to indicate that target was reached with source port and TTL value in queue for the receiver thread
        except Exception as e:
            queue.put(
                ("tcp", None, src_ports_tcp, ttl, send_time)
            )  # Store source port and TTL value in queue for the receiver thread
            s.close()
        s.close()
...
def print_results(host_ttl_results, host_delta_time):
    ...
    """
    if isinstance(
        host_ttl_results, list
    ):  # Single protocol (no protocol and list of tuples)
        for res_host, res_ttl in host_ttl_results:
            ...
            if "," in res_host:  # Several hosts for this TTL
                ...
                if host_delta_time[res_host] <= 0:
                    ...
                    print(
                        f"Hop {res_ttl}: {hop_space}{res_host}{host_fqdn} - {round(host_delta_time[res_host]*1000,2)}ms"
                    )
            else:
                print(f"Hop {res_ttl}: {hop_space}{res_host}{host_fqdn}")
        ...
        for res_ttl in sorted(host_ttl_results.keys()):
            ...
            for proto in host_ttl_results[res_ttl].keys():
                if proto == "all":  # All protocols failed to have a response
                    ...
                    for res_host in host_ttl_results[res_ttl][
                        proto
                    ]:  # For each protocol, we parse the responses per host
                            ...
                            if res_host in host_delta_time.keys():
                                if not res_host in ttl_str:  # Create a new host line
                                    if host_delta_time[res_host][proto] <= 0:
                                        ttl_str += (
                                            f"{res_host}{host_fqdn} - {proto.upper()}"
                                        )
                                    else:
...
def map_received_icmp_to_sent_udp(
    host, n_hops, host_ip, recv_host_sport, reached, queue
):
    """
    ...
    while (
        not queue.empty()
    ):  # Get all sent information by the sender thread from the queue
        (
            new_host,
            new_sport,
            new_ttl,
            send_time,
        ) = queue.get()  # new_host is None from queue
        no_resp = True
        for rhost, sport, receive_time in recv_host_sport:  # Parse ICMP responses
            if (
                sport == new_sport
            ):  # Use source port information to get associated recv_host
                no_resp = False
                new_host = rhost
                if (
                    not host_sport_ttl
                ):  # If results list is empty, let's add the first result element
                    ...
                    for (
                        ...
                        sport,
                        ttl,
                    ) in host_sport_ttl:  # Parse the already stored results
                        if (
                            new_sport == sport
                        ):  # Check if duplicate is found based on the source port (as different host could be seen due to ECMP if -r option was passed)
                            duplicate = True
                            if (
                                new_host in rhost
                            ) and new_ttl < ttl:  # If same hosts (means we went above the number of hops), compare associated TTLs and replace if TTL is lower
                                host_sport_ttl.append((new_host, new_sport, new_ttl))
                                host_sport_ttl.remove((rhost, sport, ttl))
                                host_delta_time[new_host] = receive_time - send_time
                            elif (
                                not new_host in rhost
                            ) and new_ttl == ttl:  # If different hosts and same TTL (means we got different hosts for same TTL), replace entry with one with both hosts
                                host_sport_ttl.append(
                                    (new_host + ", " + rhost, new_sport, new_ttl)
                                )
                                host_sport_ttl.remove((rhost, sport, ttl))
                                host_delta_time[new_host] = receive_time - send_time
                    if not duplicate:  # If no duplicate, just add it to the results
                        ...
                        host_delta_time[new_host] = receive_time - send_time
        if (
            no_resp and not ("* * * * * * * *", new_sport, new_ttl) in host_sport_ttl
        ):  # No response has been seen for this source port
    ...
    if reached:
        for rhost, sport, ttl in host_sport_ttl:
            if host_ip in rhost:
                ...
                host_sport_ttl[host_sport_ttl.index((rhost, sport, ttl))] = (
                    host_ip,
                    sport,
                    ttl,
                )
...
def receive_udp(
    ...
    for data, addr, receive_time in recv_data_addr:  # Parse received ICMP packets
        ...
        ip_header = data.hex()
        ip_header_len = (
            int(ip_header[1], 16) * 4 * 2
        )  # IP header length is a multiple of 32-bit (4 bytes or 4 * 2 nibbles) increment
        ...
        if (
            icmp_type == 3 and icmp_code == 3 and resp_host == host_ip
        ):  # ICMP Destination unreachable Port unreachable
            recv_host_sport.append((resp_host, inner_udp_sport, receive_time))
            reached = True

    host_ttl_results, host_delta_time = map_received_icmp_to_sent_udp(
        host, n_hops, host_ip, recv_host_sport, reached, queue
    )
...
def map_received_icmp_to_sent_tcp(
    host, n_hops, host_ip, recv_host_sport, queue, dst_port
):
    """
    ...
    while (
        ...
        (
            ...
            new_ttl,
            send_time,
        ) = queue.get()  # new_host is None from queue except when target was reached
        if new_host:  # Target was reached in sender thread
            reached = True
            new_host = host_ip
            recv_host_sport.append(
                (new_host, new_sports, new_ttl)
            )  # Let's append this as a TCP response too
        no_resp = True
        for rhost, sport, receive_time in recv_host_sport:  # Parse ICMP / TCP responses
            if (sport in new_sports) or (
                sport == new_sports
            ):  # Use source port information to get associated recv_host
                no_resp = False
                new_host = rhost
                    ...
                    for (
                        ...
                        if (sport in new_sports) or (
                            ...
                            if (
                                ...
                                host_sport_ttl.append(
                                    (new_host + ", " + rhost, new_sports, new_ttl)
                                )
                                host_sport_ttl.remove((rhost, sport, ttl))
                                host_delta_time[new_host] = receive_time - send_time
                    if not duplicate:  # If no duplicate, just add it to the results
                        ...
                        host_delta_time[new_host] = receive_time - send_time
        if (
            no_resp and not ("* * * * * * * *", new_sports, new_ttl) in host_sport_ttl
        ):  # No response has been seen for this source port
...
def receive_tcp(
    ...
    host_ttl_results, host_delta_time = map_received_icmp_to_sent_tcp(
        host, n_hops, host_ip, recv_host_sport, queue, dst_port
    )
...
def map_received_icmp_to_sent_icmp(
    host, n_hops, host_ip, recv_host_checksum_ttl, reached, queue
):
    """
    ...
    while (
        ...
        for (
            ...
            ttl,
            receive_time,
        ) in recv_host_checksum_ttl:  # Parse ICMP responses
            if (new_ttl == ttl) or (
                new_checksum == checksum
            ):  # Use TTL or checksum information to get associated recv_host
                no_resp = False
                new_host = rhost
                if (
                    not host_ttl
                    ...
                    for rhost, ttl in host_ttl:  # Parse the already stored results
                        if (
                            new_host in rhost
                        ):  # Check if duplicate is found based on the host (as different host could be seen if -r option was passed) and host
                            duplicate = True
                            if (
                                new_host in rhost
                            ) and new_ttl < ttl:  # If same hosts (means we went above the number of hops), compare associated TTLs and replace if TTL is lower
                    ...
                    if not duplicate:  # If no duplicate, just add it to the results
                        ...
                        host_delta_time[new_host] = receive_time - send_time
        if (
            no_resp and not ("* * * * * * * *", new_ttl) in host_ttl
        ):  # No response has been seen for this source port
...
def receive_icmp(
    ...
    for data, addr, receive_time in recv_data_addr:  # Parse received ICMP packets
        ...
        if icmp_type == 11 and icmp_code == 0:  # ICMP Time-to-live exceeded in transit
            ...
            inner_ip_header_len = (
                int(inner_ip_header[1], 16) * 4 * 2
            )  # Inner IP header length is a multiple of 32-bit (4 bytes or 4 * 2 nibbles) increment
            inner_icmp_header = inner_ip_header[inner_ip_header_len:]
            inner_icmp_checksum = int(inner_icmp_header[4:8], 16)
            recv_host_ttl.append(
                (resp_host, inner_icmp_checksum, None, receive_time)
            )  # Store host and inner ICMP checksum
        if (
            icmp_type == 0 and icmp_code == 0 and resp_host == host_ip
        ):  # ICMP Echo reply
            icmp_data = icmp_header[16 : len(icmp_header)]
            ttl = int(
                bytes.fromhex(icmp_data).decode().split(FLAG)[1]
            )  # Retrieve TTL from sent data
            recv_host_ttl.append(
                (resp_host, None, ttl, receive_time)
            )  # Store host and TTL
            reached = True

    host_ttl_results, host_delta_time = map_received_icmp_to_sent_icmp(
        host, n_hops, host_ip, recv_host_ttl, reached, queue
    )
...
def map_received_icmp_to_sent_all(
    timeout,
    host,
    n_hops,
    host_ip,
    ...
    while (
        ...
        if proto == "udp":
            for (
                ...
                sport,
                receive_time,
            ) in recv_host_sport_udp:  # Parse ICMP responses
                if (
                    sport == new_sport_or_checksum
                ):  # Use source port information to get associated recv_host
                    ...
                    if (
                        ...
                        host_sport_ttl.append(
                            ("udp", new_host, new_sport_or_checksum, new_ttl)
                        )
            ...
            for (
                ...
                if (new_ttl == ttl) or (
                    new_sport_or_checksum == checksum
                ):  # Use TTL or checksum information to get associated recv_host
                    no_resp_by_ttl[new_ttl][proto] = False
                    new_host = rhost
                    if (
                        ...
                        for (
                            ...
                            rhost,
                            ttl,
                        ) in host_ttl:  # Parse the already stored results
                            if (proto == "icmp") and (
                                new_host in rhost
                            ):  # Check if duplicate is found based on the host (as different host could be seen if -r option was passed) and host
                                duplicate = True
            ...
            if (
                new_host
            ):  # Target was reached in sender thread (new_host was set to True in this case)
                reached = True
                new_host = host_ip
                recv_host_sport_tcp.append(
                    (new_host, new_sport_or_checksum, 0)
                )  # Let's append this as a TCP response too
            ...
            for (
                ...
                sport,
                receive_time,
            ) in recv_host_sport_tcp:  # Parse ICMP / TCP responses
                if (sport in new_sport_or_checksum) or (
                    sport == new_sport_or_checksum
                ):  # Use source port information to get associated recv_host
                    no_resp_by_ttl[new_ttl][proto] = False
                    new_host = rhost
                    if (
                        ...
                        for (
                            ...
                            if (proto == "tcp") and (
                                (sport in new_sport_or_checksum)
                                or (sport == new_sport_or_checksum)
                            ):  # Check if duplicate is found based on the source port (as different host could be seen due to ECMP if -r option was passed)
                                duplicate = True
                                if (
                                    new_host in rhost
                        ...
                        if not duplicate:  # If no duplicate, just add it to the results
                            host_sport_ttl.append(
                                ("tcp", new_host, new_sport_or_checksum, new_ttl)
                            )
                            if not new_host in host_delta_time.keys():
                                host_delta_time[new_host] = {}
                            host_delta_time[new_host]["tcp"] = receive_time - send_time
    ...
    for ttl in no_resp_by_ttl.keys():
        if all(
            no_resp for no_resp in no_resp_by_ttl[ttl].values()
        ):  # No response has been seen for this TTL
    ...
    if reached:
        ...
        for proto, rhost, sport, ttl in host_sport_ttl:
            if host_ip in rhost:
                reached_host_ttl = ttl
                host_sport_ttl[host_sport_ttl.index((proto, rhost, sport, ttl))] = (
                    proto,
                    host_ip,
                    sport,
                    ttl,
...
def receive_all(
    ...
    for data, addr, receive_time in recv_data_addr:  # Parse received ICMP packets
        ...
        if (
            inner_ip_proto == 1 and icmp_type == 11 and icmp_code == 0
    ...
    host_ttl_results, host_delta_time = map_received_icmp_to_sent_all(
        timeout,
        host,
        n_hops,
        host_ip,
...
if __name__ == "__main__":
    ...
    if packets_to_repeat < 1 or packets_to_repeat > 16:
        print(
            f"Number of packet to send per TTL value increase must be between 1 and 16"
        )
        exit()
    ...
    match protocol:
        case "udp":
            print(
                f"flyingroutes to {host} ({host_ip}) with {n_hops} hops max ({packets_to_repeat} packets per hop) on UDP port {dst_port} with a timeout of {timeout}s"
            )
            try:
                queue = Queue()
                sync_queue = Queue()
            ...
            try:
                with Progress() as progress:
                    sender_task = progress.add_task(
                        "Sending packets...",
                        total=n_hops * packets_to_repeat,
                        visible=False,
                    )
                    receiver_task = progress.add_task(
                        "Receiving packets...",
                        total=n_hops * packets_to_repeat,
                        visible=False,
                    )
        ...
        case "tcp":
            print(
                f"flyingroutes to {host} ({host_ip}) with {n_hops} hops max ({packets_to_repeat} packets per hop) on TCP port {dst_port} with a timeout of {timeout}s"
            )
            try:
                queue = Queue()
                sync_queue = Queue()
            ...
            try:
                with Progress() as progress:
                    sender_task = progress.add_task(
                        "Sending packets...",
                        total=n_hops * packets_to_repeat * 2,
                        visible=False,
                    )
                    receiver_task = progress.add_task(
                        "Receiving packets...",
        ...
        case "all":
            print(
                f"flyingroutes to {host} ({host_ip}) with {n_hops} hops max ({packets_to_repeat} packets per hop) on ICMP, UDP port {dst_port} and TCP port {dst_port} with a timeout of {timeout}s"
            )
            try:
                queue = Queue()
                sync_queue = Queue()
            ...
            try:
                with Progress() as progress:
                    sender_task = progress.add_task(
                        "Sending packets...",
                        total=n_hops * packets_to_repeat * 3,
                        visible=False,
                    )
                    receiver_task = progress.add_task(
                        "Receiving packets...",
        ...
        case _:
            print(
                f"flyingroutes to {host} ({host_ip}) with {n_hops} hops max ({packets_to_repeat} packets per hop) on ICMP with a timeout of {timeout}s"
            )
            try:
                queue = Queue()
                sync_queue = Queue()
